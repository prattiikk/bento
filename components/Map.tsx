...
export default function MapComponent() {
    ...
    const getCoordinatesFromPlace = async (place: string) => {
        try {
            const response = await fetch(
                `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(place)}.json?access_token=pk.eyJ1IjoicHJhdHRpaWtrIiwiYSI6ImNtMjA0b2JnbDBkOTIyanF0MWlmZ3JncTAifQ._pSzkQsgp7SXiKrAsCeeiw`
            );
            ...
            if (data.features && data.features.length > 0) {
                ...
                console.log("New coordinates set:", longitude, latitude); // Debug log
            } else {
    ...
    return (
        <div className="w-full h-full relative">
            ...
            <div className=" w-full h-full mx-auto rounded-2xl overflow-hidden shadow-lg">
                <AirplaneScene />
                <Map
                    style={customMapStyle}
                    center={[lng, lat]} // Ensure this is updated with state
                    zoom={[zoom]}
                    containerStyle={{
                        height: "100%",
                        width: "100%",
            ...
            {showMenu &&
                <div className="absolute mt-2 mb-4 left-1/2 transform -translate-x-1/2 rounded-full shadow-lg ">
                    ...
                    {showSuggestions && (
                        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-lg overflow-hidden shadow-md">
                            {suggestions
                                .filter((suggestion) => suggestion.name.toLowerCase().includes(searchValue.toLowerCase()))
                                .map((suggestion, index) => (
                                    <div
                                        key={index}
                                        className="px-4 py-2 text-black hover:bg-gray-200 cursor-pointer transition-colors duration-200"
